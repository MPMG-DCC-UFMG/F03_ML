...
class PreprocessingText:
    ...
    def standarize_normalize(self, tokens):
        new_tokens, n_std, n_norm = self.ns_um.apply_both(tokens)
        return new_tokens, n_std, n_norm

    def preprocess_document(self, document):
        counter_subs = Counter()
        if self.language == 'pt':
            description = self.preprocess_document_portuguese(document)
            doc= self.tokenize_document(description)
            if self.spellcheck is not None:
                doc, n_spc = self.spellcheck_document(doc)
                counter_subs["spellcheck"] += n_spc
            if self.ns_um:
                doc, n_std, n_norm = self.standarize_normalize(doc)
                counter_subs["std"] += n_std
                counter_subs["norm"] += n_norm
            if self.lemmatize:
                doc, n_lem = self.lemmatization_document(doc)
                counter_subs["lemmatization"] += n_lem
        elif self.language == 'en':
    ...
    def preprocess_items_process(self, items, it_process, results_process, counts):
        ...
        for item in items:
            ...
            if isinstance(description, str) and description != "":
                doc, counter_subs = self.preprocess_document(description)
                counter += counter_subs
    ...
    def preprocess_items(self, items, n_process=10):
        ...
        for i in range(n_process):
            ...
            args = (items_process, i, results_process, counts))
            jobs.append(p)
            p.start()
        ...
        for i in range(n_process):
            ...
            self.counter_subs += counts[i]

        return items_descriptions

    def get_subs(self):
        return self.counter_subs
